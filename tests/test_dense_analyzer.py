...
def test_dedup_scene_anchors_on_first_relevant_not_skipped(monkeypatch, tmp_path: Path) -> None:
    """
    Scene anchoring: if a scene starts with a relevance-skipped frame, the scene summary
    must use the first lesson_relevant frame as the anchor, not the minimal skipped entry.
    """
    from pipeline.deduplicator import group_scenes
    ...
    analysis = {
        "000001": {
            ...
            "change_summary": ["UI flicker"],
            "current_state": {"visual_facts": [], "trading_relevant_interpretation": []},
        },
        "000002": {
            "frame_timestamp": "00:00:02",
            "material_change": True,
            ...
            "change_summary": ["New abstract diagram"],
            "current_state": {
                "visual_facts": ["A horizontal line represents a key level."],
                "trading_relevant_interpretation": ["Price action around level."],
            },
        },
        "000003": {
            "frame_timestamp": "00:00:03",
            "material_change": False,
            "lesson_relevant": False,
            "scene_boundary": False,
            "pipeline_status": "structural_unchanged",
        },
    }
    ...
    # first_relevant_entry must be frame 000002, not the skipped frame 000001
    relevant_entry = scene["first_relevant_entry"]
    assert relevant_entry is not None, "first_relevant_entry must not be None"
    facts = (relevant_entry.get("current_state") or {}).get("visual_facts", [])
    assert "A horizontal line represents a key level." in facts, (
        f"first_relevant_entry should be frame 000002 (the relevant one), got: {relevant_entry}"
    )


def test_dedup_scene_first_relevant_fallback_when_all_skipped(monkeypatch, tmp_path: Path) -> None:
    """
    If a scene has no lesson_relevant frames, first_relevant_entry must fall back to first_entry.
    """
    from pipeline.deduplicator import group_scenes
    ...
    analysis = {
        "000001": {
            ...
            "scene_boundary": True,
            "pipeline_status": "relevance_skipped",
            "change_summary": ["UI change"],
            "current_state": {"visual_facts": ["Some content"], "trading_relevant_interpretation": []},
        },
    }
    ...
    assert len(scenes) == 1
    scene = scenes[0]
    # first_relevant_entry must fall back to first_entry since no relevant frames exist
    assert scene["first_relevant_entry"] is scene["first_entry"], (
        "first_relevant_entry should fall back to first_entry when no relevant frames exist"
    )
