# Corpus contract v1 (Stage 6.2)

## Purpose

This contract defines the unified corpus store generated from Stage 6.1 lesson exports.
It is the ingestion boundary for later retrieval / hybrid-RAG layers.

## Required outputs

- `corpus_rule_cards.jsonl`
- `corpus_knowledge_events.jsonl`
- `corpus_evidence_index.jsonl`
- `corpus_concept_graph.json`

Additional enrichment outputs currently emitted:

- `concept_frequencies.json`
- `concept_rule_map.json`
- `concept_overlap_report.json`
- `concept_alias_registry.json`
- `rule_family_index.json`
- `corpus_metadata.json`
- `validation_report.json`

## Required corpus entities

- lessons
- knowledge events
- rule cards
- evidence refs
- concept nodes
- concept relations

## Stable ID policy

- Deterministic IDs only; no random UUIDs.
- Event/rule/evidence rows carry both corpus-level and source-level ids:
  - `global_id` — stable namespaced id (primary key for cross-lesson refs).
  - `corpus_event_id` / `corpus_rule_id` / `corpus_evidence_id` — equal to `global_id` for that row type (explicit corpus id column).
  - `source_event_id` / `source_rule_id` / `source_evidence_id` — lesson-local ids from the source JSON artifacts.
- Namespace pattern for `global_id` (and matching `corpus_*_id`):
  - `event:<lesson_slug>:<local_event_id>`
  - `rule:<lesson_slug>:<local_rule_id>`
  - `evidence:<lesson_slug>:<local_evidence_id>`
- Concept nodes are canonicalized across lessons:
  - `node:<slugified_concept_name>`
- Concept relations are deterministic from endpoints and relation type:
  - `rel:<source_node_id>:<relation_type>:<target_node_id>`

Policy helpers are documented in `pipeline/corpus/corpus_id_policy.py` (re-export from `id_utils`).

Given unchanged inputs, repeated builds must produce identical IDs and content. Lesson order in `lesson_registry.json` does not affect merged outputs (lessons are sorted by `lesson_id` before merge).

## Provenance rules

Corpus rows retain:

- source `lesson_id`
- lesson-local identifiers (`source_*_id` columns and namespaced `global_id`)
- per-row **source artifact paths** relative to the corpus root:
  - `source_knowledge_events_json`, `source_rule_cards_json`, `source_evidence_index_json`
  - `source_concept_graph_json` on concept nodes and relations in `corpus_concept_graph.json`
- `source_event_ids`, `evidence_refs`, and other cross-refs as in lesson exports
- `corpus_metadata.generated_at` as UTC ISO-8601 (validated by the corpus output validator)

## Enrichment rules

- **Concept frequencies**: lesson / rule / event / evidence counts by canonical concept node.
- **Rule families**: rules grouped by normalized `concept(+subconcept)` key.
- **Concept-to-rule mappings**: reverse index from concept node id to corpus rule IDs.
- **Cross-lesson concept overlap**: concepts appearing in more than one lesson.
- **Concept alias registry**: canonical node names + aliases + source lesson support.

## Validation rules

Validation checks include:

- required output file existence
- parseability of JSON/JSONL outputs
- uniqueness of corpus global IDs
- cross-reference integrity (rule->events/evidence, evidence->rules)
- provenance: `lesson_id`, explicit `corpus_*_id` / `source_*_id`, and source JSON paths on rows
- `corpus_metadata.generated_at` ISO-8601 when `corpus_metadata.json` is present
- when `--lesson-registry` is supplied to validate:
  - **registry replay**: every ingestible registry row (`status == valid` and `validation_status != failed`) must appear in corpus JSONL rows
  - **count roll-up**: sums of `record_counts` from those registry rows must match `len` of corpus events / rules / evidence respectively

## Versioning

- Lesson schemas remain pinned by `pipeline/contracts/schema_versions.json`.
- Corpus output compatibility for this contract is `corpus_contract_version = 1.0.0`.
- Breaking changes (required field removals/renames or semantics changes) require a new corpus contract version.

## Deferred to Stage 6.3+

- vector DB / embeddings
- retrieval API / reranking
- storage backends and serving infrastructure
- analyst UI / rule browser
