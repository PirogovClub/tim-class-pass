...
from pipeline.corpus.lesson_registry import build_registry, save_registry
...
def _rel_to_corpus_root(absolute_path: str, corpus_root: Path) -> str:
...
EPHEMERAL_REGISTRY_NAME = "_lesson_registry.ingest.v1.json"
...
def _lessons_from_registry(
    registry_path: Path,
    corpus_root: Path | None = None,
) -> tuple[list[LessonRecord], list[str]]:
    """Load lesson inputs from Stage 6.1 lesson_registry.json."""
    doc = load_registry_v1(registry_path)
    lessons: list[LessonRecord] = []
    skipped: list[str] = []
    for entry in doc.lessons:
        if entry.status != "valid" or entry.validation_status == "failed":
            skipped.append(entry.lesson_id)
            continue

        base = corpus_root.resolve() if corpus_root is not None else registry_path.parent.resolve()
        paths = {
            "knowledge_events": str((base / entry.artifacts.knowledge_events_path).resolve()),
            "rule_cards": str((base / entry.artifacts.rule_cards_path).resolve()),
            "evidence_index": str((base / entry.artifacts.evidence_index_path).resolve()),
            "concept_graph": str((base / entry.artifacts.concept_graph_path).resolve()),
        }
        available = {k: Path(v).exists() for k, v in paths.items()}
        lessons.append(
            LessonRecord(
                lesson_id=entry.lesson_id,
                lesson_slug=entry.lesson_slug or slugify_lesson_id(entry.lesson_id),
                lesson_title=entry.lesson_name or entry.lesson_id,
                source_language="ru",
                ...
                build_timestamp=entry.validated_at or "",
                content_hashes=dict(entry.artifact_hashes),
                status=entry.status,  # type: ignore[arg-type]
                warnings=[],
            )
        )
    lessons.sort(key=lambda r: r.lesson_id)
...
def build_corpus(
    ...
    effective_registry = lesson_registry_path
    if effective_registry is None:
        ingest_path = output_root / EPHEMERAL_REGISTRY_NAME
        doc = build_registry_v1(
            input_root.resolve(),
            validate=False,
            strict_validation=False,
            selected_project_roots=selected_project_roots,
        )
        save_registry_v1(doc, ingest_path)
        effective_registry = ingest_path

    lessons, skipped_registry_lessons = _lessons_from_registry(effective_registry, input_root)
    if not lessons:
    ...
    summary = {
        "lessons": len(lessons),
        "skipped_registry_lessons": skipped_registry_lessons,
        ...
        "ingestion_registry_path": str(effective_registry),
    }
    return summary
