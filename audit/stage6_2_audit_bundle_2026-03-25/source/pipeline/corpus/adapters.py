...
def globalize_event(event_dict: dict[str, Any], lesson_slug: str) -> dict[str, Any]:
    ...
    out = dict(event_dict)
    local_eid = out["event_id"]
    gid = make_global_id("event", lesson_slug, local_eid)
    out["global_id"] = gid
    out["corpus_event_id"] = gid
    out["source_event_id"] = local_eid
...
def globalize_rule(rule_dict: dict[str, Any], lesson_slug: str) -> dict[str, Any]:
    ...
    out = dict(rule_dict)
    local_rid = out["rule_id"]
    gid = make_global_id("rule", lesson_slug, local_rid)
    out["global_id"] = gid
    out["corpus_rule_id"] = gid
    out["source_rule_id"] = local_rid
    out["lesson_slug"] = lesson_slug
...
def globalize_evidence(evidence_dict: dict[str, Any], lesson_slug: str) -> dict[str, Any]:
    ...
    local_eid = out["evidence_id"]
    gid = make_global_id("evidence", lesson_slug, local_eid)
    out["global_id"] = gid
    out["corpus_evidence_id"] = gid
    out["source_evidence_id"] = local_eid
    out["lesson_slug"] = lesson_slug
