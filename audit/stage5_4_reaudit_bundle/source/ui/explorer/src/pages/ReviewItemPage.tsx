...
export function ReviewItemPage() {
  ...
  const queueBackHref = useMemo(() => {
    ...
    if (qualityTierParam) {
      q.set('qualityTier', qualityTierParam)
    }
    const s = q.toString()
    return `/review/queue${s ? `?${s}` : ''}`
  }, [queueFilter, qualityTierParam])
  ...
  async function handleNext() {
    try {
      ...
      void navigate(
        `/review/item/${encodeURIComponent(next.target_type)}/${encodeURIComponent(next.target_id)}?queueFilter=${encodeURIComponent(queueFilter)}&queueReason=${encodeURIComponent(next.queue_reason ?? '')}${tierQs}`,
        { replace: true },
      )
    } catch (e) {
      setError(e)
