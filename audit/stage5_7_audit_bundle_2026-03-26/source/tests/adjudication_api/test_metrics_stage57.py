...
def test_queue_health_deferred_rule_cards(metrics_repo: AdjudicationRepository) -> None:
    q0 = build_queue_health_metrics(metrics_repo, STANDARD_TEST_CORPUS_INDEX)
    assert q0.deferred_rule_cards == 0
    metrics_repo.append_decision_and_refresh_state(
        NewReviewDecision(
            target_type=ReviewTargetType.RULE_CARD,
            target_id="rule:q:a",
    ...
    q1 = build_queue_health_metrics(metrics_repo, STANDARD_TEST_CORPUS_INDEX)
    assert q1.deferred_rule_cards >= 1


def test_queue_health_oldest_unresolved_age_when_timestamp_present(
    metrics_repo: AdjudicationRepository,
) -> None:
    metrics_repo.append_decision_and_refresh_state(
        NewReviewDecision(
            target_type=ReviewTargetType.RULE_CARD,
    ...
    q = build_queue_health_metrics(metrics_repo, STANDARD_TEST_CORPUS_INDEX)
    assert q.unresolved_queue_size >= 1
    if q.oldest_unresolved_last_reviewed_at:
        assert q.oldest_unresolved_age_seconds is not None
        assert q.oldest_unresolved_age_seconds >= 0.0
...
def test_proposal_usefulness_counts(metrics_repo: AdjudicationRepository) -> None:
    ...
    dup_row = next(x for x in p.by_proposal_type if x.proposal_type == ProposalType.DUPLICATE_CANDIDATE.value)
    assert dup_row.accepted == 1
    assert dup_row.acceptance_rate_closed == 1.0
    merge_row = next(x for x in p.by_proposal_type if x.proposal_type == ProposalType.MERGE_CANDIDATE.value)
    assert merge_row.open == 1
    assert merge_row.acceptance_rate_closed is None


def test_proposal_dismissed_stale_superseded_counts(metrics_repo: AdjudicationRepository) -> None:
    now = utc_now_iso()
    base = dict(
        source_target_type=ReviewTargetType.RULE_CARD,
        source_target_id="rule:dup:1",
        score=0.5,
        rationale_summary="x",
        adjudication_snapshot_json="{}",
        tier_snapshot_json="{}",
    ...
    with metrics_repo.connect() as conn:
        conn.execute(
            "UPDATE adjudication_proposal SET proposal_status = ?, updated_at = ? WHERE proposal_id = ?",
            (ProposalStatus.DISMISSED.value, now, "pd1"),
        )
        conn.execute(
            "UPDATE adjudication_proposal SET proposal_status = ?, updated_at = ? WHERE proposal_id = ?",
            (ProposalStatus.STALE.value, now, "ps1"),
        )
        conn.execute(
            "UPDATE adjudication_proposal SET proposal_status = ?, updated_at = ? WHERE proposal_id = ?",
            (ProposalStatus.SUPERSEDED.value, now, "px1"),
        )
    p = build_proposal_usefulness(metrics_repo)
    assert p.dismissed_proposals >= 1
    assert p.stale_proposals >= 1
    assert p.superseded_proposals >= 1
    assert p.stale_total >= 2
    assert p.terminal_proposals >= 3
...
def test_coverage_concepts_buckets_and_ratio(metrics_repo: AdjudicationRepository) -> None:
    docs = [
        {
            "doc_id": "rule:w:1",
            "unit_type": "rule_card",
            "lesson_id": "L1",
            "canonical_concept_ids": ["concept_alpha"],
        },
        {
            "doc_id": "rule:b:1",
            "unit_type": "rule_card",
            "lesson_id": "L1",
            "canonical_concept_ids": ["concept_beta"],
        },
    ]
    ex = _FakeExplorer(docs)
    metrics_repo.append_decision_and_refresh_state(
    ...
    cov = build_coverage_concepts(metrics_repo, STANDARD_TEST_CORPUS_INDEX, ex)
    assert cov.explorer_available is True
    by_c = {b.bucket_id: b for b in cov.buckets}
    assert "concept_alpha" in by_c
    assert by_c["concept_alpha"].reviewed_not_unresolved >= 1
    assert by_c["concept_alpha"].coverage_ratio == 1.0
    assert "concept_beta" in by_c
    assert by_c["concept_beta"].reviewed_not_unresolved == 0
    assert by_c["concept_beta"].total_targets >= 1
    assert by_c["concept_beta"].coverage_ratio == 0.0
...
def test_flags_conflict_counts_and_grouping(metrics_repo: AdjudicationRepository) -> None:
    cid = "rel:node:concept_link_test:relates_to:node:other"
    rid = "rel:rule:lesson_a:r1:relates_to:rule:lesson_a:r2"
    metrics_repo.append_decision_and_refresh_state(
        NewReviewDecision(
    ...
    metrics_repo.append_decision_and_refresh_state(
        NewReviewDecision(
            ...
            target_id=rid,
            decision_type=DecisionType.RELATION_INVALID,
            reviewer_id="u1",
        ),
    )
    metrics_repo.append_decision_and_refresh_state(
        NewReviewDecision(
            target_type=ReviewTargetType.RULE_CARD,
            target_id="rule:x:2",
            decision_type=DecisionType.SPLIT_REQUIRED,
            reviewer_id="u1",
        ),
    )
    docs = [
        {
            "doc_id": "rule:x:2",
            "unit_type": "rule_card",
            "lesson_id": "L_flag_group",
            "canonical_concept_ids": ["c_flag"],
        },
    ]
    ex = _FakeExplorer(docs)
    f = build_flags_distribution(metrics_repo, STANDARD_TEST_CORPUS_INDEX, ex)
    assert f.summary.conflict_concept_invalid >= 1
    assert f.summary.conflict_relation_invalid >= 1
    assert f.summary.conflict_rule_split_required >= 1
    lesson_hits = [r for r in f.by_lesson if r.bucket_id == "L_flag_group"]
    assert lesson_hits
    assert lesson_hits[0].conflict_rule_split_required >= 1
    concept_hits = [r for r in f.by_concept if r.bucket_id == "c_flag"]
    assert concept_hits
    assert concept_hits[0].conflict_rule_split_required >= 1
