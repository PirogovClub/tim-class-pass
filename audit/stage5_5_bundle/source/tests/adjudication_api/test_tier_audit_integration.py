...
def _snapshot_all_materialized_tiers(repo: AdjudicationRepository) -> list[tuple]:
    """Snapshot stable tier columns only (resolved_at changes on every recompute)."""
    with repo.connect() as conn:
        cur = conn.execute(
            """
            SELECT target_type, target_id, tier, tier_reasons_json, blocker_codes_json,
                   is_eligible, is_promotable_to_gold, policy_version
...
def test_orphan_materialized_tier_row_purged_on_recompute(repo: AdjudicationRepository) -> None:
    repo.recompute_all_materialized_tiers(STANDARD_TEST_CORPUS_INDEX)
    template = repo.get_materialized_tier(ReviewTargetType.RULE_CARD, "rule:http:1")
    assert template is not None
    fake_id = "rule:not_in_corpus_inventory_zz"
    with repo.connect() as conn:
        conn.execute(
            """
            ...
            """,
            (
                ReviewTargetType.RULE_CARD.value,
                fake_id,
                template.tier.value,
                json.dumps(template.tier_reasons),
                json.dumps(template.blocker_codes),
                1 if template.is_eligible_for_downstream_use else 0,
                1 if template.is_promotable_to_gold else 0,
                template.resolved_at,
                template.policy_version,
            ),
        )
    assert repo.get_materialized_tier(ReviewTargetType.RULE_CARD, fake_id) is not None
    repo.recompute_all_materialized_tiers(STANDARD_TEST_CORPUS_INDEX)
    assert repo.get_materialized_tier(ReviewTargetType.RULE_CARD, fake_id) is None


def test_full_materialized_tier_table_recompute_idempotent(repo: AdjudicationRepository) -> None:
    repo.recompute_all_materialized_tiers(STANDARD_TEST_CORPUS_INDEX)
    snap1 = _snapshot_all_materialized_tiers(repo)
    repo.recompute_all_materialized_tiers(STANDARD_TEST_CORPUS_INDEX)
    snap2 = _snapshot_all_materialized_tiers(repo)
    assert snap1 == snap2
