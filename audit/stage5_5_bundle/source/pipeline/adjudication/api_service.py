"""Service layer for adjudication read/write APIs (Stage 5.2)."""

from __future__ import annotations

from typing import TYPE_CHECKING

from pipeline.adjudication.api_errors import AdjudicationApiError, ErrorCode, map_integrity_error
from pipeline.adjudication.api_models import (
    DecisionHistoryEntry,
    DecisionSubmissionRequest,
    DecisionSubmissionResponse,
    FamilyDetailResponse,
    FamilyMemberEntry,
    FamilyMembersResponse,
    ProposalDetailResponse,
    ProposalDismissRequest,
    ProposalGenerateRequest,
    ProposalGenerateResponse,
    ProposalListResponse,
    ProposalRecordResponse,
    ReviewHistoryResponse,
    ReviewItemResponse,
    TierListResponse,
    TierRecomputeResponse,
    TierStateResponse,
    TierCountsResponse,
)
from pipeline.adjudication.corpus_inventory import CorpusTargetIndex
from pipeline.adjudication.enums import (
    DecisionType,
    ProposalQueueName,
    ProposalStatus,
    ProposalType,
    QualityTier,
    ReviewTargetType,
)
from pipeline.adjudication.models import MaterializedTierRecord
from pipeline.adjudication.errors import AdjudicationIntegrityError
from pipeline.adjudication.models import NewReviewDecision, ProposalRecord, ReviewDecision
from pipeline.adjudication.repository import AdjudicationRepository

if TYPE_CHECKING:
    from pipeline.explorer.service import ExplorerService


def _decision_entry(d: ReviewDecision) -> DecisionHistoryEntry:
    return DecisionHistoryEntry(
        decision_id=d.decision_id,
        target_type=d.target_type.value,
        target_id=d.target_id,
        decision_type=d.decision_type.value,
        reviewer_id=d.reviewer_id,
        created_at=d.created_at,
        note=d.note,
        reason_code=d.reason_code,
        related_target_id=d.related_target_id,
        artifact_version=d.artifact_version,
        proposal_id=d.proposal_id,
    )


def _empty_review_item(tt: ReviewTargetType, target_id: str) -> ReviewItemResponse:
    return ReviewItemResponse(target_type=tt, target_id=target_id)


def get_review_item(repo: AdjudicationRepository, target_type: ReviewTargetType, target_id: str) -> ReviewItemResponse:
    if target_type == ReviewTargetType.CANONICAL_RULE_FAMILY:
        fam = repo.get_family(target_id)
        if fam is None:
            raise AdjudicationApiError(
                error_code=ErrorCode.NOT_FOUND,
                message=f"Canonical family {target_id!r} not found",
                status_code=404,
                details={"family_id": target_id},
            )
        decisions = repo.get_decisions_for_target(target_type, target_id)
        latest = decisions[-1].decision_type.value if decisions else None
        return ReviewItemResponse(
            target_type=target_type,
            target_id=target_id,
            current_status=fam.status.value,
            latest_decision_type=latest,
            last_reviewed_at=decisions[-1].created_at if decisions else None,
            last_reviewer_id=decisions[-1].reviewer_id if decisions else None,
            summary=fam.canonical_title,
        )

    if target_type == ReviewTargetType.RULE_CARD:
        st = repo.get_rule_card_state(target_id)
        if st is None:
            return _empty_review_item(target_type, target_id)
        return ReviewItemResponse(
            target_type=target_type,
            target_id=target_id,
            current_status=st.current_status.value if st.current_status else None,
            latest_decision_type=st.latest_decision_type.value if st.latest_decision_type else None,
            last_reviewed_at=st.last_reviewed_at,
            last_reviewer_id=st.last_reviewer_id,
            canonical_family_id=st.canonical_family_id,
            summary=st.notes_summary,
            is_duplicate=st.is_duplicate,
            duplicate_of_rule_id=st.duplicate_of_rule_id,
            is_ambiguous=st.is_ambiguous,
            is_deferred=st.is_deferred,
            is_unsupported=st.is_unsupported,
        )

    if target_type == ReviewTargetType.EVIDENCE_LINK:
        st = repo.get_evidence_link_state(target_id)
        if st is None:
            return _empty_review_item(target_type, target_id)
        return ReviewItemResponse(
            target_type=target_type,
            target_id=target_id,
            latest_decision_type=None,
            last_reviewed_at=st.last_reviewed_at,
            last_reviewer_id=st.last_reviewer_id,
            support_status=st.support_status.value if st.support_status else None,
        )

    if target_type == ReviewTargetType.CONCEPT_LINK:
        st = repo.get_concept_link_state(target_id)
        if st is None:
            return _empty_review_item(target_type, target_id)
        return ReviewItemResponse(
            target_type=target_type,
            target_id=target_id,
            last_reviewed_at=st.last_reviewed_at,
            last_reviewer_id=st.last_reviewer_id,
            link_status=st.link_status.value if st.link_status else None,
        )

    if target_type == ReviewTargetType.RELATED_RULE_RELATION:
        st = repo.get_related_rule_relation_state(target_id)
        if st is None:
            return _empty_review_item(target_type, target_id)
        return ReviewItemResponse(
            target_type=target_type,
            target_id=target_id,
            last_reviewed_at=st.last_reviewed_at,
            last_reviewer_id=st.last_reviewer_id,
            relation_status=st.relation_status.value if st.relation_status else None,
        )

    raise AdjudicationApiError(
        error_code=ErrorCode.BAD_REQUEST,
        message=f"Unsupported target_type {target_type!r}",
        status_code=400,
    )


def get_review_history(
    repo: AdjudicationRepository, target_type: ReviewTargetType, target_id: str
) -> ReviewHistoryResponse:
    if target_type == ReviewTargetType.CANONICAL_RULE_FAMILY and repo.get_family(target_id) is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.NOT_FOUND,
            message=f"Canonical family {target_id!r} not found",
            status_code=404,
            details={"family_id": target_id},
        )
    decisions = repo.get_decisions_for_target(target_type, target_id)
    return ReviewHistoryResponse(
        target_type=target_type,
        target_id=target_id,
        decisions=[_decision_entry(d) for d in decisions],
    )


def get_family_detail(repo: AdjudicationRepository, family_id: str) -> FamilyDetailResponse:
    fam = repo.get_family(family_id)
    if fam is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.NOT_FOUND,
            message=f"Family {family_id!r} not found",
            status_code=404,
            details={"family_id": family_id},
        )
    return FamilyDetailResponse(
        family_id=fam.family_id,
        canonical_title=fam.canonical_title,
        normalized_summary=fam.normalized_summary,
        status=fam.status.value,
        created_at=fam.created_at,
        updated_at=fam.updated_at,
        created_by=fam.created_by,
        primary_concept=fam.primary_concept,
        primary_subconcept=fam.primary_subconcept,
        review_completeness=fam.review_completeness,
    )


def _raise_unknown_corpus_target(
    target_type: ReviewTargetType,
    target_id: str,
    *,
    field: str = "target_id",
) -> None:
    raise AdjudicationApiError(
        error_code=ErrorCode.UNKNOWN_CORPUS_TARGET,
        message=f"Unknown corpus target {target_type.value} {target_id!r} (not in explorer inventory)",
        status_code=404,
        details={"target_type": target_type.value, "target_id": target_id, "field": field},
    )


def validate_corpus_targets_for_write(corpus_index: CorpusTargetIndex, req: DecisionSubmissionRequest) -> None:
    """Ensure primary and related rule targets exist in the corpus index before persistence."""
    if req.target_type == ReviewTargetType.CANONICAL_RULE_FAMILY:
        return
    if not corpus_index.contains(req.target_type, req.target_id):
        _raise_unknown_corpus_target(req.target_type, req.target_id)

    if req.decision_type == DecisionType.DUPLICATE_OF and req.target_type == ReviewTargetType.RULE_CARD:
        rid = req.related_target_id
        if not rid or not corpus_index.contains(ReviewTargetType.RULE_CARD, rid):
            _raise_unknown_corpus_target(
                ReviewTargetType.RULE_CARD,
                rid or "",
                field="related_target_id",
            )


def get_family_members(repo: AdjudicationRepository, family_id: str) -> FamilyMembersResponse:
    if repo.get_family(family_id) is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.NOT_FOUND,
            message=f"Family {family_id!r} not found",
            status_code=404,
            details={"family_id": family_id},
        )
    rows = repo.list_family_members(family_id)
    return FamilyMembersResponse(
        family_id=family_id,
        members=[
            FamilyMemberEntry(
                membership_id=m.membership_id,
                family_id=m.family_id,
                rule_id=m.rule_id,
                membership_role=m.membership_role.value,
                added_by_decision_id=m.added_by_decision_id,
                created_at=m.created_at,
            )
            for m in rows
        ],
    )


def _proposal_matches_decision_target(
    prop: ProposalRecord, req: DecisionSubmissionRequest
) -> bool:
    if prop.source_target_type == req.target_type and prop.source_target_id == req.target_id:
        return True
    if prop.related_target_type is not None and prop.related_target_id:
        return prop.related_target_type == req.target_type and prop.related_target_id == req.target_id
    return False


def submit_decision(
    repo: AdjudicationRepository,
    req: DecisionSubmissionRequest,
    corpus_index: CorpusTargetIndex | None,
) -> DecisionSubmissionResponse:
    from pydantic import ValidationError

    try:
        payload = NewReviewDecision(
            target_type=req.target_type,
            target_id=req.target_id,
            decision_type=req.decision_type,
            reviewer_id=req.reviewer_id,
            note=req.note,
            reason_code=req.reason_code,
            related_target_id=req.related_target_id,
            artifact_version=req.artifact_version,
            proposal_id=req.proposal_id,
        )
    except ValidationError as e:
        safe_errors = [{k: v for k, v in err.items() if k != "ctx"} for err in e.errors()]
        raise AdjudicationApiError(
            error_code=ErrorCode.VALIDATION_ERROR,
            message="Invalid decision payload",
            status_code=422,
            details={"errors": safe_errors},
        ) from e

    if req.target_type != ReviewTargetType.CANONICAL_RULE_FAMILY:
        if corpus_index is None:
            raise AdjudicationApiError(
                error_code=ErrorCode.CORPUS_INDEX_UNAVAILABLE,
                message="Corpus index is not configured; cannot validate adjudication targets.",
                status_code=503,
                details={"target_type": req.target_type.value},
            )
        validate_corpus_targets_for_write(corpus_index, req)

    if req.proposal_id:
        prop = repo.get_proposal(req.proposal_id)
        if prop is None:
            raise AdjudicationApiError(
                error_code=ErrorCode.PROPOSAL_NOT_FOUND,
                message=f"Proposal {req.proposal_id!r} not found",
                status_code=404,
                details={"proposal_id": req.proposal_id},
            )
        if prop.proposal_status != ProposalStatus.NEW:
            raise AdjudicationApiError(
                error_code=ErrorCode.PROPOSAL_INVALID_STATE,
                message=f"Proposal {req.proposal_id!r} is not open (status={prop.proposal_status.value})",
                status_code=409,
                details={"proposal_id": req.proposal_id, "status": prop.proposal_status.value},
            )
        if not _proposal_matches_decision_target(prop, req):
            raise AdjudicationApiError(
                error_code=ErrorCode.PROPOSAL_TARGET_MISMATCH,
                message="Decision target does not match proposal source or related target",
                status_code=400,
                details={
                    "proposal_id": req.proposal_id,
                    "target_type": req.target_type.value,
                    "target_id": req.target_id,
                },
            )

    try:
        d = repo.append_decision_and_refresh_state(payload)
    except AdjudicationIntegrityError as e:
        raise map_integrity_error(e) from e

    if req.proposal_id:
        repo.mark_proposal_accepted(req.proposal_id, d.decision_id)

    updated = get_review_item(repo, req.target_type, req.target_id)
    fam_summary = None
    if req.decision_type == DecisionType.MERGE_INTO and req.related_target_id:
        fam_summary = {
            "merge_into_family_id": req.related_target_id,
            "rule_id": req.target_id,
        }
    return DecisionSubmissionResponse(
        decision_id=d.decision_id,
        target_type=req.target_type,
        target_id=req.target_id,
        updated_state=updated,
        family_linkage_summary=fam_summary,
    )


def _tier_state_response(rec: MaterializedTierRecord) -> TierStateResponse:
    return TierStateResponse(
        target_type=rec.target_type,
        target_id=rec.target_id,
        tier=rec.tier.value,
        tier_reasons=rec.tier_reasons,
        blocker_codes=rec.blocker_codes,
        is_eligible_for_downstream_use=rec.is_eligible_for_downstream_use,
        is_promotable_to_gold=rec.is_promotable_to_gold,
        resolved_at=rec.resolved_at,
        policy_version=rec.policy_version,
    )


def get_tier_for_target(
    repo: AdjudicationRepository,
    target_type: ReviewTargetType,
    target_id: str,
    *,
    corpus_index: CorpusTargetIndex | None = None,
    refresh_if_missing: bool = True,
) -> TierStateResponse:
    from pipeline.adjudication.repository import TIER_MATERIALIZED_TARGET_TYPES

    if target_type not in TIER_MATERIALIZED_TARGET_TYPES:
        raise AdjudicationApiError(
            error_code=ErrorCode.BAD_REQUEST,
            message=f"Tiering not supported for target_type {target_type.value!r}",
            status_code=400,
            details={"target_type": target_type.value},
        )
    if corpus_index is not None and not corpus_index.contains(target_type, target_id):
        _raise_unknown_corpus_target(target_type, target_id)

    rec = repo.get_materialized_tier(target_type, target_id)
    if rec is None:
        if not refresh_if_missing:
            raise AdjudicationApiError(
                error_code=ErrorCode.NOT_FOUND,
                message="No tier row for target (run a decision or refresh tier)",
                status_code=404,
                details={"target_type": target_type.value, "target_id": target_id},
            )
        rec = repo.refresh_materialized_tier(target_type, target_id)
    return _tier_state_response(rec)


def list_tiers_by_tier(
    repo: AdjudicationRepository,
    tier: QualityTier,
    target_type: ReviewTargetType | None,
    limit: int,
) -> TierListResponse:
    rows = repo.list_materialized_tiers(tier=tier, target_type=target_type, limit=limit)
    return TierListResponse(
        tier=tier.value,
        target_type=target_type.value if target_type else None,
        items=[_tier_state_response(r) for r in rows],
        total=len(rows),
    )


def get_tier_counts(repo: AdjudicationRepository) -> TierCountsResponse:
    raw = repo.materialized_tier_counts()
    return TierCountsResponse(
        by_target_type=raw["by_target_type"],
        totals_by_tier=raw["totals_by_tier"],
    )


def recompute_all_tier_rows(
    repo: AdjudicationRepository, corpus_index: CorpusTargetIndex
) -> TierRecomputeResponse:
    raw = repo.recompute_all_materialized_tiers(corpus_index)
    return TierRecomputeResponse(
        total=raw["total"],
        by_target_type=raw["by_target_type"],
    )


def fetch_optional_rule_context(explorer: ExplorerService | None, doc_id: str) -> dict:
    if explorer is None:
        return {}
    try:
        detail = explorer.get_rule_detail(doc_id)
        return {"rule_detail": detail.model_dump(mode="json")}
    except Exception:
        return {}


def _proposal_record_to_response(p: ProposalRecord) -> ProposalRecordResponse:
    return ProposalRecordResponse(
        proposal_id=p.proposal_id,
        proposal_type=p.proposal_type.value,
        source_target_type=p.source_target_type.value,
        source_target_id=p.source_target_id,
        related_target_type=p.related_target_type.value if p.related_target_type else None,
        related_target_id=p.related_target_id,
        proposal_status=p.proposal_status.value,
        score=p.score,
        score_breakdown_json=p.score_breakdown_json,
        rationale_summary=p.rationale_summary,
        signals_json=p.signals_json,
        evidence_refs_json=p.evidence_refs_json,
        adjudication_snapshot_json=p.adjudication_snapshot_json,
        tier_snapshot_json=p.tier_snapshot_json,
        queue_priority=p.queue_priority,
        dedupe_key=p.dedupe_key,
        generator_version=p.generator_version,
        created_at=p.created_at,
        updated_at=p.updated_at,
        last_generated_at=p.last_generated_at,
        accepted_decision_id=p.accepted_decision_id,
        stale_reason=p.stale_reason,
    )


def _queue_name_to_proposal_type(queue_name: str | None) -> ProposalType | None:
    if not queue_name:
        return None
    try:
        qn = ProposalQueueName(queue_name)
    except ValueError:
        raise AdjudicationApiError(
            error_code=ErrorCode.VALIDATION_ERROR,
            message=f"Invalid queue_name {queue_name!r}",
            status_code=400,
            details={"queue_name": queue_name},
        ) from None
    return {
        ProposalQueueName.HIGH_CONFIDENCE_DUPLICATES: ProposalType.DUPLICATE_CANDIDATE,
        ProposalQueueName.MERGE_CANDIDATES: ProposalType.MERGE_CANDIDATE,
        ProposalQueueName.CANONICAL_FAMILY_CANDIDATES: ProposalType.CANONICAL_FAMILY_CANDIDATE,
    }.get(qn)


def generate_proposals(
    repo: AdjudicationRepository,
    corpus_index: CorpusTargetIndex,
    explorer: ExplorerService | None,
    body: ProposalGenerateRequest,
) -> ProposalGenerateResponse:
    from pipeline.adjudication.proposal_generation import (
        generate_rule_card_proposal_records,
        is_eligible_rule,
        load_rule_contexts_from_explorer,
    )
    from pipeline.adjudication.proposal_policy import DEFAULT_GENERATOR_VERSION

    if explorer is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.CORPUS_INDEX_UNAVAILABLE,
            message="Explorer service is required to load rule text for proposal generation",
            status_code=503,
            details={},
        )
    gen_ver = body.generator_version or DEFAULT_GENERATOR_VERSION
    types: list[ProposalType] | None = None
    if body.proposal_types:
        try:
            types = [ProposalType(t) for t in body.proposal_types]
        except ValueError as e:
            raise AdjudicationApiError(
                error_code=ErrorCode.VALIDATION_ERROR,
                message="Invalid proposal_types entry",
                status_code=400,
                details={"proposal_types": body.proposal_types},
            ) from e

    contexts = load_rule_contexts_from_explorer(explorer, sorted(corpus_index.rule_card_ids))
    skipped = 0
    for rid in corpus_index.rule_card_ids:
        ctx = contexts.get(rid)
        if ctx is None or not is_eligible_rule(repo, corpus_index, rid, ctx):
            skipped += 1

    records = generate_rule_card_proposal_records(
        repo,
        corpus_index,
        contexts,
        proposal_types=types,
        limit=body.limit,
        generator_version=gen_ver,
    )
    if body.dry_run:
        return ProposalGenerateResponse(
            created=len(records),
            refreshed=0,
            marked_stale=0,
            skipped=skipped,
            generator_version=gen_ver,
        )
    inv_stale = repo.mark_new_proposals_stale_when_missing_from_inventory(corpus_index)
    stats = repo.replace_or_refresh_generated_proposals(records, gen_ver)
    return ProposalGenerateResponse(
        created=stats["created"],
        refreshed=stats["refreshed"],
        marked_stale=stats["marked_stale"] + inv_stale,
        skipped=skipped,
        generator_version=gen_ver,
    )


def list_proposals_api(
    repo: AdjudicationRepository,
    *,
    proposal_type: str | None,
    proposal_status: str | None,
    source_target_type: str | None,
    source_target_id: str | None,
    queue_name: str | None,
    min_score: float | None,
    limit: int,
    offset: int,
) -> ProposalListResponse:
    pt: ProposalType | None = None
    if proposal_type:
        try:
            pt = ProposalType(proposal_type)
        except ValueError as e:
            raise AdjudicationApiError(
                error_code=ErrorCode.VALIDATION_ERROR,
                message=f"Invalid proposal_type {proposal_type!r}",
                status_code=400,
            ) from e
    q_pt = _queue_name_to_proposal_type(queue_name)
    if q_pt is not None:
        if pt is not None and pt != q_pt:
            raise AdjudicationApiError(
                error_code=ErrorCode.VALIDATION_ERROR,
                message="queue_name and proposal_type filters conflict",
                status_code=400,
                details={"proposal_type": proposal_type, "queue_name": queue_name},
            )
        pt = q_pt
    st: ReviewTargetType | None = None
    if source_target_type:
        try:
            st = ReviewTargetType(source_target_type)
        except ValueError as e:
            raise AdjudicationApiError(
                error_code=ErrorCode.VALIDATION_ERROR,
                message=f"Invalid source_target_type {source_target_type!r}",
                status_code=400,
            ) from e
    ps: ProposalStatus | None = None
    if proposal_status:
        try:
            ps = ProposalStatus(proposal_status)
        except ValueError as e:
            raise AdjudicationApiError(
                error_code=ErrorCode.VALIDATION_ERROR,
                message=f"Invalid proposal_status {proposal_status!r}",
                status_code=400,
            ) from e
    total = repo.count_proposals(
        proposal_type=pt,
        proposal_status=ps,
        source_target_type=st,
        source_target_id=source_target_id,
        min_score=min_score,
    )
    rows = repo.list_proposals(
        proposal_type=pt,
        proposal_status=ps,
        source_target_type=st,
        source_target_id=source_target_id,
        min_score=min_score,
        limit=limit,
        offset=offset,
    )
    return ProposalListResponse(
        items=[_proposal_record_to_response(r) for r in rows],
        total=total,
    )


def get_proposal_detail(repo: AdjudicationRepository, proposal_id: str) -> ProposalDetailResponse:
    p = repo.get_proposal(proposal_id)
    if p is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.PROPOSAL_NOT_FOUND,
            message=f"Proposal {proposal_id!r} not found",
            status_code=404,
            details={"proposal_id": proposal_id},
        )
    src = get_review_item(repo, p.source_target_type, p.source_target_id)
    rel = None
    if p.related_target_type and p.related_target_id:
        rel = get_review_item(repo, p.related_target_type, p.related_target_id)
    return ProposalDetailResponse(
        proposal=_proposal_record_to_response(p),
        source_item=src,
        related_item=rel,
    )


def dismiss_proposal(
    repo: AdjudicationRepository, proposal_id: str, body: ProposalDismissRequest
) -> ProposalRecordResponse:
    p = repo.get_proposal(proposal_id)
    if p is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.PROPOSAL_NOT_FOUND,
            message=f"Proposal {proposal_id!r} not found",
            status_code=404,
            details={"proposal_id": proposal_id},
        )
    if p.proposal_status != ProposalStatus.NEW:
        raise AdjudicationApiError(
            error_code=ErrorCode.PROPOSAL_INVALID_STATE,
            message=f"Proposal {proposal_id!r} is not open",
            status_code=409,
            details={"proposal_id": proposal_id, "status": p.proposal_status.value},
        )
    repo.mark_proposal_dismissed(proposal_id, body.note)
    out = repo.get_proposal(proposal_id)
    if out is None:
        raise AdjudicationApiError(
            error_code=ErrorCode.NOT_FOUND,
            message="Proposal disappeared after dismiss",
            status_code=500,
        )
    return _proposal_record_to_response(out)
