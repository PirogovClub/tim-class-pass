...
def _is_promotable_to_gold(tier: QualityTier, blockers: list[str]) -> bool:
    """True only if current adjudicated state could reach Gold without contradicting that state.

    Not a 'might improve someday if humans re-decide' flag — duplicate-capped and rejected
    paths are never promotable under the current rule_card semantics.
    """
    if tier in (QualityTier.GOLD, QualityTier.UNRESOLVED):
        return False
    b = set(blockers)
    if DUPLICATE_CAPS_TIER in b or REJECTED_STATE in b:
        return False
    structural_block = {
        NO_ADJUDICATION,
        NEEDS_REVIEW,
    ...
    if b & structural_block:
        return False
    return tier in (QualityTier.SILVER, QualityTier.BRONZE)
...
def _record(
    ...
    *,
    ...
    promotable = _is_promotable_to_gold(tier, blockers)
