...
def validate_registry_v1(
    ...
    *,
    ...
    """Check registry paths, counts, hashes, and version fields vs frozen schema_versions.json."""
    ...
    for entry in doc.lessons:
        ...
        for label, rel in (
            ...
            if not path.is_file():
                ...
                continue
            key = f"{label}_path"
            h = entry.artifact_hashes.get(key) or entry.artifact_hashes.get(label)
            if h:
                import hashlib
