...
def test_fails_when_rule_lacks_source_event_ids_key(
    corpus_lesson_minimal: Path,
    corpus_lesson_minimal_paths: dict[str, Path],
) -> None:
    p = corpus_lesson_minimal_paths["rule_cards"]
...
def test_fails_when_cited_knowledge_event_lacks_timestamps(
    corpus_lesson_minimal: Path,
    corpus_lesson_minimal_paths: dict[str, Path],
) -> None:
    p = corpus_lesson_minimal_paths["knowledge_events"]
    data = json.loads(p.read_text(encoding="utf-8"))
    data["events"][0]["timestamp_start"] = ""
    data["events"][0]["timestamp_end"] = ""
    p.write_text(json.dumps(data, indent=2), encoding="utf-8")
    lessons = discover_lessons(corpus_lesson_minimal)
    out = validate_lesson_record_v1(lessons[0], strict=True)
    assert not out.passed
    assert any(e["category"] == "timestamp" for e in out.errors)


def test_fails_when_evidence_with_frames_lacks_timestamps(
    corpus_lesson_minimal: Path,
    corpus_lesson_minimal_paths: dict[str, Path],
) -> None:
    p = corpus_lesson_minimal_paths["evidence_index"]
    data = json.loads(p.read_text(encoding="utf-8"))
    del data["evidence_refs"][0]["timestamp_start"]
    del data["evidence_refs"][0]["timestamp_end"]
    p.write_text(json.dumps(data, indent=2), encoding="utf-8")
    lessons = discover_lessons(corpus_lesson_minimal)
    out = validate_lesson_record_v1(lessons[0], strict=True)
    assert not out.passed
    assert any("Evidence" in e["message"] and "timestamp" in e["category"] for e in out.errors)


def test_registry_validation_status_mismatch_after_disk_change(
    corpus_lesson_minimal: Path,
    tmp_path: Path,
    corpus_lesson_minimal_paths: dict[str, Path],
) -> None:
    doc = build_registry_v1(corpus_lesson_minimal, validate=True, strict_validation=True)
    assert doc.lessons[0].validation_status == "passed"
    reg_path = tmp_path / "stale_registry.json"
    save_registry_v1(doc, reg_path)
    p = corpus_lesson_minimal_paths["rule_cards"]
    data = json.loads(p.read_text(encoding="utf-8"))
    del data["rules"][0]["lesson_id"]
    p.write_text(json.dumps(data, indent=2), encoding="utf-8")
    stale = load_registry_v1(reg_path)
    rep = validate_registry_v1(stale, corpus_lesson_minimal, strict=True)
    assert any(i["category"] == "validation_status_mismatch" for i in rep["issues"])
