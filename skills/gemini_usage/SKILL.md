---
name: Gemini usage
description: How to use the Gemini API in this project — env, per-video config, scripts, the main pipeline, the standalone markdown pipeline, and the central client.
---

# Gemini Usage

Use this skill when configuring or running the main pipeline, the standalone markdown pipeline, or `pipeline/gap_detector` / `pipeline/vlm_translator` with **Gemini** as the agent or provider. All Gemini calls go through the central **`helpers/clients/gemini_client.py`** module.
...
|-------------|----------------------|-----------|
| **pipeline/main.py** (main pipeline) | `--agent gemini` or `--agent-images gemini`, or set `agent_images` in `data/<video_id>/pipeline.yml` or env | Step 2 (frame analysis) can call Gemini; Step 3 markdown synthesis always uses Gemini via the shared client |
...
**Main pipeline precedence:** `data/<video_id>/pipeline.yml` (`default`) **>** env **>** step default.

- **Main pipeline config**  
  Under `default` in `data/<video_id>/pipeline.yml`, you can set:
  - `model_name` — fallback for all steps.
  - `model_images`, `model_component2`, `model_gaps`, `model_vlm` — per-step overrides.
  Example:
  ```yaml
  default:
    agent_images: gemini
...
For long gap-detection runs, set **`GEMINI_STREAMING=1`** in env. Gap detection will use streaming and then parse JSON once at the end. Default is non-streaming.
...
- **`helpers/clients/gemini_client.py`** provides:
  ...
  - `get_model_for_step(step, video_id=None)` — model string for step `"images"` | `"gaps"` | `"vlm"`.
