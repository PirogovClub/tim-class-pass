...
When you run this project's pipeline (`uv run main.py --video_id ...` or `--url ...`), run it **from beginning to end**.
...
4. Re-run the same command (`uv run main.py --video_id ...` or `--url ...`).
...
1. Run once with **--parallel**: `uv run main.py --video_id <id> --parallel`. Step 1 runs (extract frames); Step 2 generates all batch task files (`batches/task_*.json`) and `batches/manifest.json`, then exits 10.
