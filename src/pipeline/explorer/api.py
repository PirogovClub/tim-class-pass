...
def get_explorer_service_optional() -> ExplorerService | None:
    """Return explorer service if initialized (for optional cross-module use, e.g. adjudication bundle)."""
    return _explorer_service
...
@explorer_router.post("/compare/rules", response_model=RuleCompareResponse)
def browser_compare_rules(req: RuleCompareRequest) -> RuleCompareResponse:
    service = _get_explorer()
    try:
        return service.compare_rules(req.rule_ids, include_related_context=req.include_related_context)
    except KeyError as exc:
        raise HTTPException(404, f"Document {exc.args[0]!r} not found") from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@explorer_router.post("/compare/lessons", response_model=LessonCompareResponse)
def browser_compare_lessons(req: LessonCompareRequest) -> LessonCompareResponse:
    service = _get_explorer()
    try:
        return service.compare_lessons(req.lesson_ids)
    except KeyError as exc:
        raise HTTPException(404, f"Lesson {exc.args[0]!r} not found") from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc


@explorer_router.post("/compare/units", response_model=UnitCompareResponse)
def browser_compare_units(req: UnitCompareRequest) -> UnitCompareResponse:
    service = _get_explorer()
    try:
        return service.compare_units(req.items)
    except KeyError as exc:
        raise HTTPException(404, f"Document {exc.args[0]!r} not found") from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
@explorer_router.get("/event/{doc_id}", response_model=EventDetailResponse)
def browser_event(doc_id: str) -> EventDetailResponse:
    service = _get_explorer()
    try:
        return service.get_event_detail(doc_id)
    except KeyError:
        raise HTTPException(404, f"Document {doc_id!r} not found") from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
@explorer_router.get("/rule/{doc_id}/related", response_model=RelatedRulesResponse)
def browser_related_rules(doc_id: str) -> RelatedRulesResponse:
    service = _get_explorer()
    try:
        return service.get_related_rules(doc_id)
    except KeyError:
        raise HTTPException(404, f"Document {doc_id!r} not found") from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
@explorer_router.get("/concept/{concept_id}/rules", response_model=ConceptRuleListResponse)
def browser_concept_rules(concept_id: str) -> ConceptRuleListResponse:
    service = _get_explorer()
    try:
        return service.get_concept_rules(concept_id)
    except KeyError:
        raise HTTPException(404, f"Concept {concept_id!r} not found") from None


@explorer_router.get("/concept/{concept_id}/lessons", response_model=ConceptLessonListResponse)
def browser_concept_lessons(concept_id: str) -> ConceptLessonListResponse:
    service = _get_explorer()
    try:
        return service.get_concept_lessons(concept_id)
    except KeyError:
        raise HTTPException(404, f"Concept {concept_id!r} not found") from None
