...
from pipeline.rag.config import ALL_UNIT_TYPES, UnitType as RAGUnitType
...
def _provenance_dict(doc: dict[str, Any]) -> dict[str, Any]:
...
def build_rule_detail(
    ...
    *,
    ...
    return RuleDetailResponse(
        ...
        subconcept=doc.get("subconcept"),
        canonical_concept_ids=_concept_ids(doc),
        provenance=_provenance_dict(doc),
        rule_text=doc.get("rule_text") or "",
        rule_text_ru=doc.get("rule_text_ru") or "",
        conditions=list(doc.get("conditions") or []),
        invalidation=list(doc.get("invalidation") or []),
...
def build_event_detail(
    ...
    *,
    ...
    return EventDetailResponse(
        ...
        confidence_score=doc.get("confidence_score"),
        canonical_concept_ids=_concept_ids(doc),
        linked_evidence=[build_result_card(candidate) for candidate in linked_evidence],
        linked_rules=[build_result_card(candidate) for candidate in linked_rules],
        linked_events=[build_result_card(candidate) for candidate in linked_events],
    )


def _unit_compare_row(doc: dict[str, Any]) -> UnitCompareRow:
    ut = str(doc.get("unit_type") or "")
    if ut not in set(ALL_UNIT_TYPES):
        raise ValueError(f"Unsupported unit_type {ut!r}")
    return UnitCompareRow(
        doc_id=str(doc.get("doc_id") or ""),
        unit_type=cast(RAGUnitType, ut),
        lesson_id=str(doc.get("lesson_id") or ""),
        title=_first_nonempty(doc.get("title"), str(doc.get("doc_id") or "")),
        summary=_result_snippet(doc),
        timestamps=list(doc.get("timestamps") or []),
        canonical_concept_ids=_concept_ids(doc),
        evidence_ids=[str(x) for x in doc.get("evidence_ids") or []],
        source_event_ids=[str(x) for x in doc.get("source_event_ids") or []],
        source_rule_ids=[str(x) for x in doc.get("source_rule_ids") or []],
        event_type=doc.get("event_type") if ut == "knowledge_event" else None,
        support_basis=doc.get("support_basis"),
        teaching_mode=doc.get("teaching_mode"),
        evidence_requirement=doc.get("evidence_requirement"),
        provenance=_provenance_dict(doc),
...
def build_unit_compare_response(docs: list[dict[str, Any]]) -> UnitCompareResponse:
    return UnitCompareResponse(items=[_unit_compare_row(doc) for doc in docs])
