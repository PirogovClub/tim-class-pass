...
class EventDetailResponse(BaseModel):
    ...
    confidence_score: float | None = None
    canonical_concept_ids: list[str] = Field(default_factory=list)
    linked_evidence: list[BrowserResultCard] = Field(default_factory=list)
    linked_rules: list[BrowserResultCard] = Field(default_factory=list)
    linked_events: list[BrowserResultCard] = Field(default_factory=list)


class UnitCompareItemRef(BaseModel):
    unit_type: UnitType
    doc_id: str


class UnitCompareRequest(BaseModel):
    items: list[UnitCompareItemRef] = Field(default_factory=list)


class UnitCompareRow(BaseModel):
    doc_id: str
    unit_type: UnitType
    lesson_id: str = ""
    title: str = ""
...
class UnitCompareResponse(BaseModel):
    items: list[UnitCompareRow] = Field(default_factory=list)
