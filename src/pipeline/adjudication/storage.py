...
-- Stage 5.4: materialized quality tier per adjudication target
CREATE TABLE IF NOT EXISTS materialized_tier_state (
    target_type TEXT NOT NULL,
    target_id TEXT NOT NULL,
    tier TEXT NOT NULL,
    ...
    is_promotable_to_gold INTEGER NOT NULL,
    resolved_at TEXT NOT NULL,
    policy_version TEXT NOT NULL,
    PRIMARY KEY (target_type, target_id)
);

CREATE INDEX IF NOT EXISTS idx_materialized_tier_tier
    ON materialized_tier_state (tier);
CREATE INDEX IF NOT EXISTS idx_materialized_tier_type_tier
    ON materialized_tier_state (target_type, tier);

-- Stage 5.5: durable proposal-assisted review
...
CREATE INDEX IF NOT EXISTS idx_adjudication_proposal_type_status
    ON adjudication_proposal (proposal_type, proposal_status);
CREATE INDEX IF NOT EXISTS idx_adjudication_proposal_source
    ON adjudication_proposal (source_target_type, source_target_id);
CREATE INDEX IF NOT EXISTS idx_adjudication_proposal_related
    ON adjudication_proposal (related_target_type, related_target_id);
CREATE INDEX IF NOT EXISTS idx_adjudication_proposal_queue_priority
    ON adjudication_proposal (queue_priority DESC);
CREATE INDEX IF NOT EXISTS idx_adjudication_proposal_generator
    ON adjudication_proposal (generator_version);
"""
