...
### 3.4 Transcript / timeline timestamps (contract v1 overlay)

Pydantic allows optional `timestamp_start` / `timestamp_end` on some models; **contract v1** additionally requires:

- Every **KnowledgeEvent** whose `event_id` appears in any `rule_cards[].source_event_ids` must have **non-empty** `timestamp_start` and `timestamp_end`.
- Every **EvidenceRef** that has `frame_ids`, `screenshot_paths`, or `linked_rule_ids` must have **non-empty** `timestamp_start` and `timestamp_end`.

This satisfies §10.2 (“timestamps where required”) for rule-linked transcript units and visual evidence.
...
**Per-artifact `schema_version` strings** inside JSON files (e.g. `"1.0"`) are legacy/export markers. The **canonical** semantic versions for corpus tooling are the frozen keys in `schema_versions.json`, echoed on each registry row.

**Backward compatibility:** consumers must accept **patch**-level additions of optional fields. Removing keys, renaming keys, or tightening required fields is a **breaking** change and requires a new `lesson_contract_version` and updated validator.
...
5. Missing **contract v1 timestamps** (see §3.4) on cited knowledge events or tied evidence refs.
6. In **strict** mode, selected **warnings** (empty artifacts, version stamp drift vs frozen file, integrity warnings) are promoted to errors.

**Registry replay:** `validate_registry_v1` re-runs live lesson validation for each row where `validation_status` is `passed` or `failed` and reports `validation_status_mismatch` if the registry no longer matches disk.
