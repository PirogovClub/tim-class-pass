...
def build_registry_v1(
    ...
    *,
    ...
    for lr in records:
        ...
        # When validation is skipped, treat complete artifact sets as valid for ingestion (e.g. ephemeral registries).
        if validate:
            status = "valid" if all_four and validation_status == "passed" else "invalid"
        else:
            status = "valid" if all_four else "invalid"
        if not all_four:
            status = "invalid"
