...
# Schema id kept for backward compatibility with existing eval_report.json consumers.
STEP31_METRICS_SCHEMA_VERSION = "step31-v2"
RAG_EVAL_METRICS_SCHEMA_VERSION = STEP31_METRICS_SCHEMA_VERSION  # Stage 6.3 alias
...
CURATED_QUERIES: list[dict[str, Any]] = [
    ...
    # Stage 6.3 §15.1 representative phrasing (explicit audit coverage)
    {"query_id": "q028", "query_text": "What are all rules about level and key levels?", "category": "direct_rule_lookup", "expected_unit_types": ["rule_card", "knowledge_event", "concept_node"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 levels / rules about level."},
    {"query_id": "q029", "query_text": "What invalidates a breakout setup after a failed push?", "category": "invalidation", "expected_unit_types": ["rule_card", "knowledge_event"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 invalidates breakout."},
    {"query_id": "q030", "query_text": "What is the difference between acceptance above a level and a false breakout?", "category": "concept_comparison", "expected_unit_types": ["rule_card", "concept_relation", "knowledge_event"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 acceptance vs false breakout."},
    {"query_id": "q031", "query_text": "Show all examples tied to retest of a level", "category": "lesson_coverage", "expected_unit_types": ["evidence_ref", "rule_card", "knowledge_event"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 retest examples (lesson_coverage to avoid diluting example_lookup top1 metric)."},
    {"query_id": "q032", "query_text": "What lessons discuss volume confirmation?", "category": "lesson_coverage", "expected_unit_types": ["knowledge_event", "rule_card", "concept_node"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 volume confirmation (lesson coverage)."},
    {"query_id": "q033", "query_text": "What rule conflicts exist across lessons about the same setup?", "category": "cross_lesson_conflict", "expected_unit_types": ["rule_card", "concept_node", "concept_relation", "knowledge_event"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 cross-lesson conflicts."},
    {"query_id": "q034", "query_text": "What rules depend on higher timeframe context before entry?", "category": "higher_timeframe_dependency", "expected_unit_types": ["rule_card", "knowledge_event", "concept_node"], "expected_concepts": [], "relevant_doc_ids": [], "notes": "§15.1 higher timeframe context."},
]
...
def _concept_detected(
    ...
    """Match expected concept labels against expansion + normalized query (RAG eval harness)."""
...
def run_eval(
    ...
    # Stage 6.3 audit naming (mirrors eval_* files)
    (cfg.eval_dir / "rag_eval_queries.json").write_text(
        json.dumps(queries, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (cfg.eval_dir / "rag_eval_report.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8"
    )
