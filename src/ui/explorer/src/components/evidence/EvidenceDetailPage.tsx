...
export function EvidenceDetailPage({ data, loading = false }: { data?: EvidenceDetailResponse; loading?: boolean }) {
  ...
  return <PageContainer><div className="space-y-6"><div data-testid="evidence-doc-id" className="text-xs text-slate-500">Evidence <code className="rounded bg-slate-100 px-1">{data.doc_id}</code></div><EntityHeader title={data.title} unitType="evidence_ref" lessonId={data.lesson_id} /><SupportBadges supportBasis={data.support_basis} confidenceScore={data.confidence_score} /><div className="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-700"><p><strong>Strength:</strong> {data.evidence_strength ?? 'n/a'}</p><p className="mt-2"><strong>Role:</strong> {data.evidence_role_detail ?? 'n/a'}</p></div><section className="rounded-xl border border-slate-200 bg-white p-5"><EvidenceContext snippet={data.snippet} visualSummary={data.visual_summary} lessonId={data.lesson_id} frameIds={data.frame_ids} /></section><section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-lg font-semibold text-slate-900">Timestamps</h2><div className="mt-3"><TimestampList timestamps={data.timestamps} /></div></section><EvidenceLinkedRules cards={data.source_rules} /><EvidenceLinkedEvents cards={data.source_events} /><ProvenanceSection provenance={data.provenance} /></div></PageContainer>;
}
