...
export function ScreenshotModal({
  ...
  return (
    <ScreenshotModalContent
      key={`${lessonId}:${initialIndex}:${frameIds.join('|')}`}
      open={open}
      onClose={onClose}
      lessonId={lessonId}
      frameIds={frameIds}
...
function ScreenshotModalContent({
  open,
  onClose,
  lessonId,
  frameIds,
  initialIndex,
}: Omit<ScreenshotModalProps, 'initialIndex'> & { initialIndex: number }) {
  const titleId = useId()
...
function ScreenshotImage({ imageUrl, frameLabel }: { imageUrl: string; frameLabel: string }) {
