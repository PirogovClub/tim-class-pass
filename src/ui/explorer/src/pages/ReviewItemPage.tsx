...
export function ReviewItemPage() {
  ...
  const qualityTierParam = searchParams.get('qualityTier') ?? ''
  const proposalIdParam = searchParams.get('proposalId')?.trim() || null
  const navigate = useNavigate()
  const queueFilter = parseQueueFilter(searchParams.get('queueFilter'))
  const reviewQueue = parseReviewBackendQueue(searchParams.get('reviewQueue'))
  ...
  const queueBackHref = useMemo(
    () => buildReviewQueueBackHref(searchParams),
    [searchParams],
  )

  const optionalQualityTierForApi = useMemo(() => {
    const p = QualityTierSchema.safeParse(qualityTierParam)
  ...
  async function handleNext() {
    try {
      const next = await getNextQueueItem(queueFilter, reviewQueue, {
        qualityTier: optionalQualityTierForApi,
      })
      if (!next) {
        setError(new Error('No next item in queue.'))
        return
      }
      const tierQs = qualityTierParam ? `&qualityTier=${encodeURIComponent(qualityTierParam)}` : ''
      const rq =
      ...
      void navigate(
        `/review/item/${encodeURIComponent(next.target_type)}/${encodeURIComponent(next.target_id)}?queueFilter=${encodeURIComponent(queueFilter)}&queueReason=${encodeURIComponent(next.queue_reason ?? '')}${tierQs}${rq}${pid}`,
        { replace: true },
      )
    } catch (e) {
      setError(e)
      ...
      {bundle && !loading && !error ? (
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6 lg:col-span-2">
            ...
            <QualityTierPanel bundle={bundle} />

            <ProposalPanel bundle={bundle} />
          ...
          <div className="space-y-6">
            <DecisionPanel bundle={bundle} onSubmitted={load} proposalId={proposalIdParam} />
          </div>
        </div>
      ) : null}
    </PageContainer>
