...
import { postRagSearchExplain } from '@/lib/api/rag'
...
export function SearchPage() {
  ...
  const [explainTrace, setExplainTrace] = useState<string | null>(null)
  ...
  async function runRetrievalExplain() {
    if (!state.q.trim()) {
      return
    }
    setExplainBusy(true)
    ...
    try {
      const res = await postRagSearchExplain({
        query: state.q.trim(),
        top_k: state.topK,
        unit_types: state.filters.unit_types,
        lesson_ids: state.filters.lesson_ids,
        concept_ids: state.filters.concept_ids,
        min_confidence_score: state.filters.min_confidence_score,
        return_summary: true,
        require_evidence: false,
      })
      setExplainTrace(JSON.stringify(res.retrieval_trace, null, 2))
    } catch (err) {
  ...
  return (
    <PageContainer>
      <div className="flex flex-col gap-6">
        <header className="rounded-lg border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm text-slate-700">
          <strong className="text-slate-900">Stage 6.4 analyst browser</strong> — hybrid search with filters,
          multi-item compare, and optional retrieval explain (lexical / vector / graph signals).
        </header>
          ...
          <div className="min-w-0 flex-1 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={!state.q.trim() || explainBusy}
                onClick={() => void runRetrievalExplain()}
              >
                {explainBusy ? 'Explaining…' : 'Explain retrieval'}
              </Button>
              {multiCompare.refs.length >= 2 ? (
                <Link
                  to="/compare/units"
                  data-testid="open-compare-units"
                  className="inline-flex h-8 items-center justify-center rounded-md bg-slate-900 px-3 text-sm font-medium text-white hover:bg-slate-800"
                >
                  Compare selected ({multiCompare.refs.length})
                </Link>
              ) : (
                <span className="text-xs text-slate-500">
                  Multi-compare: add 2–4 hits via &quot;Compare&quot; on cards.
                </span>
              )}
            </div>
            {explainError ? (
            ...
            {explainTrace ? (
              <details className="rounded-md border border-slate-200 bg-white p-3 text-xs" data-testid="retrieval-explain-panel">
                <summary className="cursor-pointer font-medium text-slate-800">Retrieval trace (RAG /explain)</summary>
                <pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap text-slate-800">{explainTrace}</pre>
              </details>
            ) : null}
          </div>
        </div>
