...
class Operator:
    ...
    def run_action(self, button_name: str) -> None:
...
def test_project_detail_shows_step_and_group_actions(page: Page, live_ui_server):
    ...
    expect(page.get_by_text("Step 1. Dense Capture")).to_be_visible()
    expect(page.get_by_role("button", name="Run Step 1 dense capture")).to_be_visible()
    expect(page.get_by_role("button", name="Run Prepare project")).to_be_visible()
    expect(page.get_by_role("button", name="Run all with batch")).to_be_visible()
...
def test_operator_can_create_project_reconcile_batch_and_build_corpus(page: Page, live_ui_server):
    ...
    operator.run_action("Run all with batch")
...
def test_operator_can_cancel_waiting_remote_run(page: Page, live_ui_server):
    ...
    operator.run_action("Run Prepare project")
    operator.wait_for_run_status("SUCCEEDED")
    page.goto(f"{live_ui_server['base_url']}/dashboard")
    page.get_by_role("link", name="Cancel Browser Lesson").click()
    operator.run_action("Run Remote batch vision only")
    operator.wait_for_run_status("WAITING_REMOTE")
    page.reload()
